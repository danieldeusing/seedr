import type { CanonicalCodingAgent, CodingAgent, ComponentType, PluginType } from "@seedr/shared";

/**
 * The one runtime vocabulary of coding-agent identifiers (plan §5). Everything
 * that parses, validates or matches an agent id goes through here.
 */
...
/** The agent id for a known value, or null for an unknown one. */
export function canonicalAgent(value: unknown): CanonicalCodingAgent | null {
  return isCanonicalAgent(value) ? value : null;
}
...
/** Whether one agent can hold one content type. */
