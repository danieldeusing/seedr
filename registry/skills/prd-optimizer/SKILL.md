...
```json
"baselineChecks": {"Cross-ref symmetry": 4, "Terminology": 5},
"dimensions": {"Consistency": ["Cross-ref symmetry", "Terminology"]}
```
...
**4. Re-score** — The scoring subagent (*Who scores*, Phase 3) scores ALL harness checks against the modified document. Use the structure map to read only affected sections — you don't need to re-read the entire document if the change was localized. Use the exact same anchored rubrics from the frozen harness.
