...
export function StatusBar() {
  ...
  return (
    <footer className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-card text-[11px]" data-testid="status-bar">
      {/* Chrome, not content: the bar spans the full viewport, edge to edge, independent of
          the page's own max-w-[var(--content-w)] column - matching footer.status everywhere
          else in the estate. Wraps onto two rows on phones: the path collapses first, the
          controls never clip. */}
      <div className="flex min-h-8 w-full flex-wrap items-center justify-between gap-x-4 gap-y-1 px-4 py-1">
