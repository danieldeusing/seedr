...
| plugin | copilot | `extraKnownMarketplaces[<mp>] = {"source":{"source":"directory","path":<tree>}}` beside `installed-plugins/<mp>/<name>/`, for every plugin — not only first-party. The tree's `.claude-plugin/marketplace.json` (created if missing) and, when the tree already ships one, `.github/plugin/marketplace.json` (which takes precedence and carries the per-plugin `skills`/`commands`/`hooks` keys) are both aligned to the marketplace name. The `github` source (`{"source":"github","repo":…}`) survives only for a plugin installed by reference alone (no tree, e.g. Codex-only). Pointing Copilot at a marketplace repository fetches and validates that file whole; Anthropic's marketplaces fail that validation (object `source` values, descriptions past 1024 characters), so no plugin from them installed until the digest-verified tree itself became a one-plugin marketplace. Because the install is a directory, not a marketplace reference, **`copilot plugin update` does not refresh it — re-run `seedr add` to pick up a new revision.** | `copilot plugin marketplace add <dir>` on 1.0.80 wrote exactly that; a direct `plugin install <dir>` works too but the CLI announces it deprecated. Confirmed for a non-first-party plugin (`ponytail`, from the published registry, no manual `copilot plugin install`): a Live Plugin loaded from the directory, never copied |
